...
import { setLocalRelation } from '../../core/sync/localFirst';

export async function getUniversities() {
  const { data, error } = await supabase
    .from('universities').select('*').eq('active', true).order('name');
  if (error) return [];
  return data || [];
}
...
export async function getUniversityById(id) {
  if (!id) return null;
  const { data, error } = await supabase
    .from('universities').select('*').eq('id', String(id)).maybeSingle();
...
export async function setStudentInterest(targetId, interested) {
  ...
  if (interested) {
    ...
    await supabase.from('student_interests').delete()
      .eq('user_id', userId).eq('target_id', String(targetId));
  }
}
...
export async function getStudentInterests(userId = activeUserKey()) {
  if (!userId) return [];
  const { data, error } = await supabase
    .from('student_interests').select('target_id').eq('user_id', String(userId));
  if (error) return [];
  return (data || []).map((r) => String(r.target_id));
}

// --- Student recommendations ------------------------------------------
// Server-side matching keeps the candidate pool private and enforces the
// two-shared-preferences rule. The UI uses excludeIds during refreshes so
// only a few cards are rotated instead of replacing the whole 12-card list.
export async function getStudentRecommendations({ limit = 12, excludeIds = [] } = {}) {
  ...
  return (data || []).map((row) => ({
    ...
    budget: row.budget || '',
    accommodationPreference: row.accommodation_preference || 'Any',
    roommatesNeeded: row.roommates_needed || '1',
    ...
    createdAt: null,
    grad: ['#6E63B8', '#3E3670'],
    lifestyle: Array.isArray(row.preference_tags) ? row.preference_tags : [],
    sharedPreferenceCount: Number(row.shared_preference_count || 0),
    matchedPreferences: Array.isArray(row.matched_preferences) ? row.matched_preferences : [],
