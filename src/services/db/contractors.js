...
// Module-level cache — mirrors the properties feed cache's reasoning
// (avoid re-fetching the whole contractor catalog on every render), but
// contractors don't need a TTL: there's no realtime subscription for this
// table, so the cache only ever needs invalidating explicitly (see
// resetContractorCache, called from db/account.js's clearCache and after a
// contractor registration changes the catalog).
let CONTRACTORS = null;

export function resetContractorCache() {
  CONTRACTORS = null;
}
...
function rowToContractor(row, userProfile = null) {
  ...
  return {
    ...
    business: String(row.business_name || 'Contractor'),
    category: String(row.primary_trade || 'General'),
    city: String(row.city || 'Harare'),
    area: String(row.area || areas[0] || 'Unknown'),
    rating: numOr(row.rating, 0),
    jobs: intOr(row.jobs, 0),
    verified: row.verification === 'verified',
    verificationStatus: row.verification || 'pending',
    ...
    freeQuotes: Boolean(row.free_quotes),
  };
}
...
export async function setContractorLike(contractorId, liked) {
  ...
  await setLocalRelation('contractorLikes', {
    userId, itemId: String(contractorId), liked,
...
export async function getContractorRegistrations(userId = null) {
