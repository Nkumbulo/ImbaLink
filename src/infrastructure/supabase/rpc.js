...
export async function rpc(name, args = {}, options = {}) {
  if (!name) {
    throw new AppError("RPC name is required.", { code: "RPC_NAME_REQUIRED" });
  }
  ...
  if (error) {
    ...
    wrapped.details = {
      ...(wrapped.details || {}),
      rpc: name,
      args: options.includeArgsInError ? args : undefined,
    };
...
export async function invokeFunction(name, body, _options = {}) {
  if (!name) {
    throw new AppError("Function name is required.", {
  ...
  if (data?.error) {
    throw new AppError(data.error, {
      code: data.code || "FUNCTION_REJECTED",
      details: { function: name },
    });
  }
