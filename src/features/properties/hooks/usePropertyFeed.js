...
export function usePropertyFeed({ city = 'All', filters = {}, query = '', limit = 24, hydrated = false, landlordListings = [], setLandlordListings }) {
  ...
  const createLandlordListing = useCallback(async (input) => {
    const record = await createLandlordListingApi(input);
  ...
  const updateLandlordListing = useCallback(async (propertyId, input) => {
    const record = await updateLandlordListingApi(propertyId, input);
