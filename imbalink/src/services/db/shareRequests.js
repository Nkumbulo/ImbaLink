...
import { requireUser, requireCurrentUserId } from './shared/identity';
...
function rowToShareRequest(row) {
  ...
  return {
    ...
    roommatesNeeded: String(row.roommates_needed ?? 1),
    budget: row.budget || '',
    moveInDate: row.move_in_date || '',
    preferences: row.preferences || '',
    preferenceTags: Array.isArray(row.preference_tags) ? row.preference_tags : [],
...
export async function createShareRequest(input) {
  ...
  const record = {
    ...
    preference_tags: Array.isArray(input?.preferenceTags)
      ? input.preferenceTags.filter((t) => typeof t === 'string').slice(0, 12)
      : (Array.isArray(existing?.preferenceTags) ? existing.preferenceTags : []),
  ...
  const { data, error } = await supabase
    .from('student_share_requests').upsert(record, { onConflict: 'id' }).select();
  if (error) throw error;
  return rowToShareRequest(data?.[0]);
}

export async function getShareRequests(userId = null) {
...
export async function getUniversityGeneralShareRequestStudents(universityName, excludeUserId = null) {
  const university = String(universityName || '').trim();
  if (!university) return [];

  const { data: requests, error: requestError } = await supabase
    .from('student_share_requests')
    .select('*')
  ...
  const userIds = [...new Set(requests.map((r) => String(r.user_id || '')).filter(Boolean))]
  ...
  return requests.map((request) => {
    ...
    return {
      ...
      university,
      studyYear: details.studyYear || '',
      area: details.preferredArea || '',
      city: details.preferredCity || '',
      budget: request.budget || details.budget || '',
      roommatesNeeded: String(request.roommates_needed ?? details.roommatesNeeded ?? 1),
      ...
      utilitiesIncluded: request.utilities_included || '',
      importantNotes: request.important_notes || '',
      verificationStatus: ['unverified','pending','verified','rejected'].includes(studentProfile?.verification_status)
        ? studentProfile.verification_status : 'unverified',
      propertyId: null,
      createdAt: request.created_at || null,
      serverCompatibilityScore: 0,
...
export async function getActiveShareRequestsForProperty(propertyId) {
  if (propertyId == null) return [];
  const { data, error } = await supabase
    .from('student_share_requests').select('*')
    .eq('property_id', String(propertyId)).eq('status', 'active');
  if (error) return [];
  return (data || []).map(rowToShareRequest).filter(Boolean);
}

export async function getMyShareRequestForProperty(userId, propertyId) {
...
export async function setShareRequestStatus(id, status) {
  const { data, error } = await supabase
    .from('student_share_requests')
...
export async function withdrawShareRequest(id) {
  return setShareRequestStatus(id, 'withdrawn');
}

export async function deleteShareRequest(id) {
  const { error } = await supabase.from('student_share_requests').delete().eq('id', String(id));
  if (error) throw error;
  return true;
}

export async function getShareRequestCountsByProperty() {
  const { data, error } = await supabase
    .from('student_share_requests').select('property_id').eq('status', 'active');
  if (error) return {};
  const counts = {};
  for (const r of data || []) {
